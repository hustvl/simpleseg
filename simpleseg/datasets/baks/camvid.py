...
def add_items(img_path, mask_path, aug_img_path, aug_mask_path, mode, maxSkip=0):
    ...
    for it in c_items:
        item = (os.path.join(img_path, it), os.path.join(
            mask_path, it.replace('.png', '_L.png')))
        items.append(item)
        if mode != 'test' and maxSkip > 0:
            ...
            prev_item = (os.path.join(aug_img_path, prev_it),
                         os.path.join(aug_mask_path, prev_it))
            next_item = (os.path.join(aug_img_path, next_it),
                         os.path.join(aug_mask_path, next_it))
...
def make_dataset(root, quality, mode, maxSkip=0, cv_split=0, hardnm=0):
    ...
    mask_path = os.path.join(root, original_img_dir, 'train_labels')
    aug_img_path = os.path.join(root, augmented_img_dir, 'train')
    aug_mask_path = os.path.join(root, augmented_img_dir, 'train_labels')

    train_items, train_aug_items = add_items(
        img_path, mask_path, aug_img_path, aug_mask_path, mode, maxSkip)
    logger.info(
        'Camvid has a total of {} train images'.format(len(train_items)))

    img_path = os.path.join(root, original_img_dir, 'val')
    mask_path = os.path.join(root, original_img_dir, 'val_labels')
    aug_img_path = os.path.join(root, augmented_img_dir, 'val')
    aug_mask_path = os.path.join(root, augmented_img_dir, 'val_labels')

    val_items, val_aug_items = add_items(
        img_path, mask_path, aug_img_path, aug_mask_path, mode, maxSkip)
    logger.info(
        'Camvid has a total of {} validation images'.format(len(val_items)))

    if mode == 'test':
        img_path = os.path.join(root, original_img_dir, 'test')
        mask_path = os.path.join(root, original_img_dir, 'test_labels')
        test_items, test_aug_items = add_items(
            img_path, mask_path, aug_img_path, aug_mask_path, mode, maxSkip)
        logger.info(
            'Camvid has a total of {} test images'.format(len(test_items)))
    ...
    logger.info('Camvid-{}: {} images'.format(mode, len(items)))
...
class CamVid(data.Dataset):
    ...
    def __init__(self, root, quality, mode, maxSkip=0, joint_transform=None,
        ...
        self.imgs, self.aug_imgs = make_dataset(
            root, quality, mode, self.maxSkip, cv_split=self.cv_split, hardnm=self.hardnm)
        assert len(self.imgs), 'Found 0 images, please check the data set'

        # Centroids for GT data
        if self.class_uniform_pct > 0:
            json_fn = 'camvid_tile{}_cv{}_{}.json'.format(
                self.class_uniform_tile, self.cv_split, self.mode)
            ...
            if self.maxSkip > 0:
                json_fn = 'camvid_tile{}_cv{}_{}_skip{}.json'.format(
                    self.class_uniform_tile, self.cv_split, self.mode, self.maxSkip)
                if os.path.isfile(json_fn):
                    with open(json_fn, 'r') as json_data:
                        centroids = json.load(json_data)
                    self.aug_centroids = {
    ...
    def __getitem__(self, index):
        ...
        img, mask = Image.open(img_path).convert(
            'RGB'), convert_label(Image.open(mask_path))
