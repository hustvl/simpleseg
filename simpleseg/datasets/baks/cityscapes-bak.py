...
def add_items(root, items, aug_items, cities, img_path, mask_path, mask_postfix, mode, maxSkip):
    """
...
def make_cv_splits(img_dir_name, root):
    """
...
def make_test_split(root, img_dir_name):
...
def make_dataset(root, quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    """
    ...
    if quality == 'coarse':
        ...
        logger.info('{} coarse cities: '.format(
            mode) + str(coarse_dirs[mode]))
        add_items(root, items, aug_items, coarse_dirs[mode], img_path, mask_path,
        ...
        cv_splits = make_cv_splits(img_dir_name, root)
        if mode == 'trainval':
            modes = ['train', 'val']
        else:
            modes = [mode]
        for mode in modes:
            if mode == 'test':
                cv_splits = make_test_split(root, img_dir_name)
                add_items(root, items, aug_items, cv_splits, img_path, mask_path,
                          mask_postfix, mode, maxSkip)
            else:
                logger.info('{} fine cities: '.format(
                    mode) + str(cv_splits[cv_split][mode]))
                add_items(root, items, aug_items, cv_splits[cv_split][mode], img_path, mask_path,
                          mask_postfix, mode, maxSkip)
    else:
        raise 'unknown cityscapes quality {}'.format(quality)

    logger.info('Cityscapes-{}: {} images'.format(mode,
                                                  len(items) + len(aug_items)))
    return items, aug_items


def make_dataset_video(root):
    """
...
class Cityscapes(data.Dataset):
    ...
    def __init__(self, root, quality, mode, maxSkip=0, joint_transform=None, sliding_crop=None,
        ...
        if cv_split:
            ...
            assert cv_split < CV_SPLITS, \
                'expected cv_split {} to be < CV_SPLITS {}'.format(
                    cv_split, CV_SPLITS)
        else:
            self.cv_split = 0
        self.imgs, _ = make_dataset(
            root, quality, mode, self.maxSkip, cv_split=self.cv_split)
        if len(self.imgs) == 0:
...
class CityScapesVideo(data.Dataset):
    ...
    def __init__(self, root, transform=None):
        self.imgs = make_dataset_video(root)
        if len(self.imgs) == 0:
            raise RuntimeError('Found 0 images, please check the data set')
        self.transform = transform
...
class CityScapesUniform(data.Dataset):
    ...
    def __init__(self, root, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        if self.coarse_boost_classes is not None:
            ...
            if os.path.isfile(json_fn):
                ...
                self.coarse_imgs, _ = make_dataset(
                    root, 'coarse', mode, cv_split=0)
    ...
    def cities_uniform(self, imgs, name):
        ...
        logger.info('Cities for {} '.format(name) + str(sorted(city_names)))
