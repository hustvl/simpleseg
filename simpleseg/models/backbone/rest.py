# code borrowed from https://github.com/wofmanaf/ResT/blob/main/models/rest.py
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, in_ch=3, out_ch=768, with_pos=False, norm="BN"):
        super().__init__()
...
class BasicStem(nn.Module):
    def __init__(self, in_ch=3, out_ch=64, with_pos=False, norm="BN"):
        super(BasicStem, self).__init__()
        hidden_ch = out_ch // 2
        self.conv1 = nn.Conv2d(
            in_ch, hidden_ch, kernel_size=3, stride=2, padding=1, bias=False)
        self.norm1 = get_norm(norm, hidden_ch)
        self.conv2 = nn.Conv2d(hidden_ch, hidden_ch,
                               kernel_size=3, stride=1, padding=1, bias=False)
        self.norm2 = get_norm(norm, hidden_ch)
...
class ResT(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 norm_layer=nn.LayerNorm, apply_transform=False, batchnorm="BN"):
        super().__init__()
        ...
        self.stem = BasicStem(
            in_ch=in_chans, out_ch=embed_dims[0], with_pos=True, norm=batchnorm)

        self.patch_embed_2 = PatchEmbed(
            patch_size=2, in_ch=embed_dims[0], out_ch=embed_dims[1], with_pos=True, norm=batchnorm)
        self.patch_embed_3 = PatchEmbed(
            patch_size=2, in_ch=embed_dims[1], out_ch=embed_dims[2], with_pos=True, norm=batchnorm)
        self.patch_embed_4 = PatchEmbed(
            patch_size=2, in_ch=embed_dims[2], out_ch=embed_dims[3], with_pos=True, norm=batchnorm)
    ...
    def init_weights(self, pretrained=None):
        ...
        def _init_weights(m):
            ...
            elif isinstance(m, (nn.LayerNorm, nn.BatchNorm2d, SyncBatchNorm)):
...
def build_rest_lite(cfg):

    model = ResT(
        embed_dims=cfg.MODEL.RESTLITE.EMBED_DIM,
        depths=cfg.MODEL.RESTLITE.DEPTH,
        num_heads=cfg.MODEL.RESTLITE.HEADS,
        mlp_ratios=[4, 4, 4, 4],
        qkv_bias=True,
        sr_ratios=[8, 4, 2, 1],
        apply_transform=True,
        batchnorm=cfg.MODEL.RESTLITE.BATCH_NORM)
